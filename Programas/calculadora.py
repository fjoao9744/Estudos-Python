...
botao_sqrt = Button(sqrt_frame, text="√", font=("Arial", 15))
...
botao_comma = Button(comma_frame, text=".", font=("Arial", 15))
...
# consertar
    for i, _ in enumerate(equacao):

        if _ in "+-/X":
            new_equacao.append(equacao[0 : equacao.index(_)])
            new_equacao.append(_)
            equacao = equacao[equacao.index(_) + 1:]
    new_equacao.append(equacao)

    print(new_equacao)
    for i, _ in enumerate(new_equacao):
        match _:
            case "+":
                resultado += int(new_equacao[new_equacao.index(_) - 1])
        print(new_equacao)
        new_equacao.pop(new_equacao.index(_) + 1)
        if new_equacao[new_equacao.index(_)] == '+':
                resultado += int(new_equacao[new_equacao.index(_) + 1])
